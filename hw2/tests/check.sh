...
assert first == second, "повторные активации различаются"
...
profile = memory_profile(load_params())
pids = [run['pid'] for item in profile for run in item['runs']]
assert len(pids) == len(set(pids)), "режимы или повторы используют один процесс"
peak = {item["mode"]: item["peak_mb"] for item in profile}
...
assert peak + 0.1 >= data['weights_bytes'] / 1024**2, "пик меньше базовых весов"
