...
def forward_hooks(modules: dict):
    """Наблюдать выходы и снять только свои хуки, в том числе при исключении."""
...
def device_allocated_bytes(device: torch.device) -> int:
    """Метрика устройства; на CUDA — встроенный пик аллокатора."""
    if device.type == "mps":
...
class PeakMemory:
    """CPU/CUDA: встроенный пик. MPS: максимум опросов драйвера и границ фаз.

    У MPS нет используемого здесь встроенного счётчика пика. Опрос может
    пропустить короткую аллокацию; интервал и метод явно попадают в отчёт.
    """
    ...
    def __exit__(self, *exc) -> bool:
        ...
        if self.error is not None and exc[0] is None:
            raise RuntimeError("Не удалось опросить память MPS") from self.error
        return False
    ...
    def result(self) -> dict:
        ...
        return {
            ...
            "peak_method": "sampled maximum" if self.device.type == "mps" else "high-water mark",
            "poll_interval_seconds": self.interval if self.device.type == "mps" else None,
        }
...
def measure_mode(mode: str, params: dict) -> dict:
    ...
    with PeakMemory(device, params["memory"].get("poll_interval", 0.01)) as peak:
        ids = torch.randint(
...
def memory_profile(params: dict) -> list[dict]:
    ...
    for mode in MODES:
        ...
        worst["runs"] = [dict(item) for item in runs]
...
def environment(params: dict, memory: list[dict]) -> dict:
    ...
    return {
        ...
        "num_threads": params["runtime"]["num_threads"],
        "model_revision": params["model"].get("revision", "main"),
...
def main() -> None:
    ...
    parser.add_argument("--params-stdin", action="store_true", help="служебная передача конфигурации")
