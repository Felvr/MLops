...
if ! $RUN dvc repro > "$SANDBOX.log" 2>&1; then
  fail "dvc repro v1 завершился с ошибкой"
  cat "$SANDBOX.log"
fi
...
if ! $RUN dvc repro > "$SANDBOX.log" 2>&1; then
  fail "dvc repro v2 завершился с ошибкой"
  cat "$SANDBOX.log"
fi
...
restore_params
if ! $RUN dvc repro > "$SANDBOX.log" 2>&1; then
  fail "не удалось восстановить исходную версию пайплайна"
  cat "$SANDBOX.log"
fi
rm -f "$SANDBOX.log"
